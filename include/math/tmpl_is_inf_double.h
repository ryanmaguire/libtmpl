...
tmpl_Bool tmpl_Double_Is_Inf(double x)
{
     ...
     *  TMPL_DOUBLE_IS_NAN_OR_INF macro checks the exponent value, and the    *
     *  TMPL_DOUBLE_IS_NAN macro checks the mantissa.                         */
    if (TMPL_DOUBLE_IS_NAN_OR_INF(w) && !TMPL_DOUBLE_IS_NAN(w))
        return tmpl_True;

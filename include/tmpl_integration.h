...
/******************************************************************************
 *  Function:                                                                 *
 *      tmpl_Double_Filon01_Integrand                                         *
 *  Purpose:                                                                  *
 *      Numerically integrates f(t) exp(i g(t)) dt across a small bin where   *
 *      f(t) is assumed real and constant, and g(t) = a t + b.                *
 *  Arguments:                                                                *
 *      value (const double):                                                 *
 ...
 *          The integral of value * exp(i g(x)) across the bin, assuming      *
 *          g(x) = ax + b for constants a and b.                              *
 *  Notes:                                                                    *
 *      Float and long double equivalents are provided as well.               *
 ******************************************************************************/
extern tmpl_ComplexDouble
tmpl_Double_Filon01_Integrand(const double value,
                              const double g_left,
...
tmpl_LDouble_Filon01_Integrand(const long double value,
 ...
 *  Purpose:                                                                  *
 *      Numerically integrates f(t) exp(i g(t)) dt across a small bin where   *
 *      f(t) is assumed constant across each bin, and g(t) = a t^2 + b t + c. *
 *  Arguments:                                                                *
...
extern tmpl_ComplexFloat
tmpl_CFloat_Filon02_Integrand(const tmpl_ComplexFloat left,
                              const tmpl_ComplexFloat right,
                              const float g_left,
                              const float g_middle,
                              const float g_right,
                              const float width);

extern tmpl_ComplexLongDouble
tmpl_CLDouble_Filon02_Integrand(const tmpl_ComplexLongDouble left,
                                const tmpl_ComplexLongDouble right,
                                const long double g_left,
                                const long double g_middle,
                                const long double g_right,
                                const long double width);
 ...
 *  Purpose:                                                                  *
 *      Numerically integrates f(t) exp(i g(t)) dt across a small bin where   *
 *      f(t) = A t + B (A and B are complex), and g(t) = a t + b.             *
 *  Arguments:                                                                *
 *      left (const tmpl_ComplexDouble):                                      *
 *          The value for f at the left end of the bin.                       *
 *      right (const tmpl_ComplexDouble):                                     *
 ...
 ******************************************************************************/
extern tmpl_ComplexDouble
tmpl_CDouble_Filon11_Integrand(const tmpl_ComplexDouble left,
                               const tmpl_ComplexDouble right,
                               const double g_left,
                               const double g_right,
                               const double width);
 ...
 *  Purpose:                                                                  *
 *      Numerically integrates f(t) exp(i g(t)) dt across two bins where      *
 *      f(t) = A t + B across each bin, and g(t) = a t^2 + b t + c.           *
 *  Arguments:                                                                *
 *      left (const tmpl_ComplexDouble):                                      *
...
tmpl_CDouble_Filon12_Integrand(const tmpl_ComplexDouble left,
                               const tmpl_ComplexDouble middle,
                               const tmpl_ComplexDouble right,
                               const double g_left,
                               const double g_middle,
                               const double g_right,
...
extern tmpl_ComplexFloat
tmpl_CFloat_Filon12_Integrand(const tmpl_ComplexFloat left,
                              const tmpl_ComplexFloat middle,
                              const tmpl_ComplexFloat right,
                              const float g_left,
                              const float g_middle,
                              const float g_right,
...
extern tmpl_ComplexLongDouble
tmpl_CLDouble_Filon12_Integrand(const tmpl_ComplexLongDouble left,
                                const tmpl_ComplexLongDouble middle,
                                const tmpl_ComplexLongDouble right,
                                const long double g_left,
                                const long double g_middle,
                                const long double g_right,

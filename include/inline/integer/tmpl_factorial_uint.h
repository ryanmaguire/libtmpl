...
#define TMPL_UINT_FACTORIAL_USE_LOOKUP_TABLE 1
...
#define TMPL_UINT_FACTORIAL_USE_LOOKUP_TABLE 0

#endif
/*  End of #if TMPL_UINT_BIT == 16.                                           */

/*  For supported widths, this function is very short. Just use the table.    */
#if TMPL_UINT_FACTORIAL_USE_LOOKUP_TABLE == 1

/******************************************************************************
 *                            Lookup Table Version                            *
 ******************************************************************************/

/*  Unsigned int factorial function.                                          */
TMPL_INLINE_DECL
unsigned int tmpl_UInt_Factorial(const unsigned int n)
{
    /*  If n can't index the lookup table, we assume that n! is greater than  *
     *  the maximum value of unsigned int. Return zero in this case. n! is    *
     *  never zero for non-negative integers, so this signals the caller that *
     *  an error has occurred.                                                */
    if (n >= TMPL_ARRAY_SIZE(tmpl_uint_factorial_values))
        return 0U;
...
#endif
/*  End of #if TMPL_UINT_FACTORIAL_USE_LOOKUP_TABLE == 1.                     */

/*  Undefine everything in case someone wants to #include this file.          */
#undef TMPL_UINT_FACTORIAL_USE_LOOKUP_TABLE

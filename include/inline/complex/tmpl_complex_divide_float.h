...
tmpl_CFloat_Divide(tmpl_ComplexFloat z, tmpl_ComplexFloat w)
{
    ...
    const float abs_w_real = tmpl_Float_Abs(w.dat[0]);
    const float abs_w_imag = tmpl_Float_Abs(w.dat[1]);
    ...
    if (abs_w_real > abs_w_imag)
    {
        ...
        const float ratio = w.dat[1] / w.dat[0];
        const float denom = w.dat[0] + w.dat[1] * ratio;
    ...
    else
    {
        ...
        const float ratio = w.dat[0] / w.dat[1];
        const float denom = w.dat[0] * ratio + w.dat[1];

 ...
 *      the hypot function (tmpl_CLDouble_Quick_Divide now provides the       *
 *      faster algorithm that is susceptible to overflow and underflow).      *
 ******************************************************************************/
...
tmpl_CLDouble_Divide(tmpl_ComplexLongDouble z, tmpl_ComplexLongDouble w)
{
    ...
    const long double norm_w = tmpl_LDouble_Hypot(w.dat[0], w.dat[1]);

    /*  Multiplication is faster than division. Pre-compute the reciprocal.   */
    const long double rcpr_norm_w = 1.0L / norm_w;
    ...
    const long double wn_x = w.dat[0] * rcpr_norm_w;
    const long double wn_y = w.dat[1] * rcpr_norm_w;

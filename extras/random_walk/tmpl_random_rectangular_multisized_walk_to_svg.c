...
static struct pair random_pair(void)
{
    ...
    int rint = (int)(4.0 * rreal);
    struct pair out;
    rreal = (double)rand() / (double)RAND_MAX;
...
int main(void)
{
    ...
    fprintf(fp, "<circle cx=\"%f\" cy=\"%f\" r=\"5\" stroke=\"black\" fill=\"#ffffff\"/>\n",
            xscale * (A[0].x - xmin), yscale * (A[0].y - ymin));
    fprintf(fp, "<circle cx=\"%f\" cy=\"%f\" r=\"5\" stroke=\"black\" fill=\"#ffffff\"/>\n",
            xscale * (A[walk_size - 1U].x - xmin),
            yscale * (A[walk_size - 1U].y - ymin));
    free(A);
    fprintf(fp, "</g>\n</svg>\n");
    return 0;
}

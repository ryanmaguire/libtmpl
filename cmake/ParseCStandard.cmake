...
function(set_c_standard)
    ...
    # Parse STDVER Strip off the "-std=" flag and leading/trailing whitespace
    string(STRIP "${ARG_STDVER}" C_STANDARD_CHOICE)
    string(REGEX REPLACE "-std[ \t\r\n]*=[ \t\r\n]*" "" C_STANDARD_CHOICE ${C_STANDARD_CHOICE})
    ...
    if(C_STANDARD_CHOICE IN_LIST C90_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 90 C_EXTENSIONS FALSE)
    elseif(C_STANDARD_CHOICE IN_LIST C90_EXTENDED_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 90 C_EXTENSIONS TRUE)
    elseif(C_STANDARD_CHOICE IN_LIST C99_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 99 C_EXTENSIONS FALSE)
    elseif(C_STANDARD_CHOICE IN_LIST C99_EXTENDED_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 99 C_EXTENSIONS TRUE)
    elseif(C_STANDARD_CHOICE IN_LIST C11_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 11 C_EXTENSIONS FALSE)
    elseif(C_STANDARD_CHOICE IN_LIST C11_EXTENDED_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 11 C_EXTENSIONS TRUE)
    elseif(C_STANDARD_CHOICE IN_LIST C17_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 17 C_EXTENSIONS FALSE)
    elseif(C_STANDARD_CHOICE IN_LIST C17_EXTENDED_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 17 C_EXTENSIONS TRUE)
    elseif(C_STANDARD_CHOICE IN_LIST C23_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 23 C_EXTENSIONS FALSE)
    elseif(C_STANDARD_CHOICE IN_LIST C23_EXTENDED_FLAGS)
        set_target_properties(${ARG_TARGET} PROPERTIES C_STANDARD 23 C_EXTENSIONS TRUE)
    else()
        message(WARNING "Unrecognized STDVER value: \"${ARG_STDVER}\". Defaulting to c90")
        set_property(TARGET ${ARG_TARGET} PROPERTY C_STANDARD 90)
    endif()
    if(${ARG_ENABLE_VERBOSE})
        cmake_print_properties(TARGETS ${ARG_TARGET} PROPERTIES C_STANDARD C_EXTENSIONS)
    endif()
endfunction()

 ...
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Draw a circle with a black-to-white gradient giving it the appearance *
 *      of a 3D sphere.                                                       *
 ******************************************************************************/

/*  tmpl_Binary_PGM_From_Data function found here.                            */
...
/*  Function for drawing a sphere with a black-to-white gradient.             */
int main(void)
{
    ...
    /*  Allocate memory for the PGM's data.                                   */
    unsigned char *data = malloc(sizeof(*data) * width * height);
...
#pragma omp parallel for
    ...
    for (n = 0U; n < width*height; ++n)
    {
         ...
         *  based on the distance of the point to the center of the circle.   */
            ...
            data[n] = (unsigned char)(UCHAR_MAX * (1.0 - dist));
    }
    /*  End of for-loop for n.                                                */

    /*  Create the PGM from the data.                                         */
    tmpl_Binary_PGM_From_Data("tmpl_sphere.pgm", data, width, height);

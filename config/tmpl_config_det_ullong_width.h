 ...
 ******************************************************************************
 *                        tmpl_config_det_ullong_width                        *
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Determines the width of unsigned long long int.                       *
 ******************************************************************************
 *                             DEFINED FUNCTIONS                              *
 ******************************************************************************
 *  Function Name:                                                            *
 ...
 *  Purpose:                                                                  *
 *      Determines the number of numerical bits in unsigned long long int.    *
 *  Arguments:                                                                *
 *      None (void).                                                          *
 *  Output:                                                                   *
 *      None (void).                                                          *
...
#ifndef TMPL_CONFIG_DET_ULLONG_WIDTH_H
#define TMPL_CONFIG_DET_ULLONG_WIDTH_H

/*  First check if long long support is available.                            */
#ifdef TMPL_LONG_LONG_IS_AVAILABLE

/*  Globals for the config file are all found here.                           */
#include "tmpl_config_globals.h"

/*  The width of unsigned char can be used to compute the size of long long.  */
#include "tmpl_config_det_uchar_width.h"

/*  Function for determining the number of bits in unsigned long long int.    */
static void tmpl_det_ullong_width(void)
{
    ...
    const unsigned long long int two = 2ULL;
     ...
     *  bits in unsigned long long int.                                       */
    while (value)
    {
        value = two * value;
        ++tmpl_ullong_width;
     ...
     *  the number of bits in unsigned long long int using the code above.    *
     *  Set the Boolean to true.                                              */
    tmpl_ullong_width_is_known = 1U;

    /*  The number of bits in unsigned long long can be obtained from the     *
     *  number of bits in unsigned char (which does not allow padding) and    *
     *  using the sizeof operator. The C standard allows unsigned long long   *
     *  to have padding, hence it is possible for width != number_of_bits,    *
     *  but this is rare.                                                     */
    ...
    tmpl_number_of_bits_in_ullong =
        sizeof(unsigned long long int) * tmpl_uchar_width;
}
/*  End of tmpl_det_ullong_width.                                             */

#endif
/*  End of #ifdef TMPL_LONG_LONG_IS_AVAILABLE.                                */

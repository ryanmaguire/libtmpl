...
tmpl_IntVector *tmpl_Create_Zero_IntVector(size_t length)
{
    ...
    vec->data = TMPL_CALLOC(signed int, length);
...
tmpl_UIntVector *tmpl_Create_Zero_UIntVector(size_t length)
{
    /*  Declare a variable for the vector pointer we're returning.            */
    tmpl_UIntVector *vec;
    ...
    /*  Otherwise, allocate memory for the data pointer. calloc, unlike       *
     *  malloc, will initialize everything to zero, giving us a zero vector.  */
    vec->data = TMPL_CALLOC(unsigned int, length);

 ...
 *          numbers. Since asin(x) = x to single-precision for such small     *
...
#ifndef TMPL_ARCSIN_SIMD_FLOAT_H
#define TMPL_ARCSIN_SIMD_FLOAT_H
...
/*  Near-branchless single-precision inverse sine (asin equivalent).          */
TMPL_SIMD_DECL
TMPL_CONST_FUNC
...
TMPL_UNSEQUENCED
{
    ...
    const float asin_tail_x_pos = tmpl_float_pi_by_two - 2.0F * sqrt_u * s;
    const float asin_tail_x_neg = -asin_tail_x_pos;
    ...
    const float asin_tail_x = (x < 0.0F ? asin_tail_x_neg : asin_tail_x_pos);
    ...
    const float out = (abs_x >= 0.5F ? asin_tail_x : asin_small_x);

...
void tmpl_PolynomialQ_Shrink(tmpl_PolynomialQ *poly)
{
    ...
    for (n = 0UL; n <= poly->degree; ++n)
        if (!tmpl_RationalNumber_Is_Zero(poly->coeffs[n]))
            last = n;

 ...
 *  Called Functions:                                                         *
 *      __builtin_ctz (Unix-Users, if available)                              *
 *          Count-Trailing-Zeros. Returns the number of trailing zeros in the *
 *          binary represenation of an unsigned int.                          *
 *      _BitScanForward (Windows users)                                       *
 *          Windows count-trailing-zeros function.                            *
 *      tmpl_UInt_Trailing_Zeros    (tmpl_number_theory.h)                    *
 *          Function that counts the number of trailing zeros in an unsigned  *
 *          int. This is used if __builtin_ctz and _BitScanForward are        *
 *          not available.                                                    *
 *  Method:                                                                   *
...
/*  The binary and mixed-binary algorithms can take advantage of a hardware   *
 *  ctz or bsf function, if available. Check for this.                        */
#if TMPL_GCD_ALGORITHM == 1 || TMPL_GCD_ALGORITHM == 2
...
#endif
/*  End of #if TMPL_GCD_ALGORITHM == 1 || TMPL_GCD_ALGORITHM == 2.            */

...
tmpl_ComplexDouble tmpl_CDouble_Cos(tmpl_ComplexDouble z)
{
    ...
    double sin_x, cos_x, sinh_y, cosh_y;
    tmpl_ComplexDouble cos_z;

    /*  Compute (cos(x), sin(x)) and (cosh(y), sinh(y)), simultaneously.      */
    tmpl_Double_SinCos(z.dat[0], &sin_x, &cos_x);
    tmpl_Double_SinhCosh(z.dat[1], &sinh_y, &cosh_y);

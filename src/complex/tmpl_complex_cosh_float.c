...
/*  Single precision complex hyperbolic cosine (ccoshf equivalent).           */
tmpl_ComplexFloat tmpl_CFloat_Cosh(tmpl_ComplexFloat z)
{
    /*  Declare necessary variables. C89 requires declarations at the top.    */
    float sin_y, cos_y, sinh_x, cosh_x;
    tmpl_ComplexFloat cos_z;

    /*  Compute (cos(y), sin(y)) and (cosh(x), sinh(x)), simultaneously.      */
    tmpl_Float_SinCos(z.dat[1], &sin_y, &cos_y);
    tmpl_Float_SinhCosh(z.dat[0], &sinh_x, &cosh_x);

...
/*  Long double precision complex hyperbolic cosine (ccoshl equivalent).      */
tmpl_ComplexLongDouble tmpl_CLDouble_Cosh(tmpl_ComplexLongDouble z)
{
    /*  Declare necessary variables. C89 requires declarations at the top.    */
    long double sin_y, cos_y, sinh_x, cosh_x;
    tmpl_ComplexLongDouble cos_z;

    /*  Compute (cos(y), sin(y)) and (cosh(x), sinh(x)), simultaneously.      */
    tmpl_LDouble_SinCos(z.dat[1], &sin_y, &cos_y);
    tmpl_LDouble_SinhCosh(z.dat[0], &sinh_x, &cosh_x);

...
tmpl_ComplexLongDouble tmpl_CLDouble_Cos(tmpl_ComplexLongDouble z)
{
    ...
    long double sin_x, cos_x, sinh_y, cosh_y;
    tmpl_ComplexLongDouble cos_z;

    /*  Compute (cos(x), sin(x)) and (cosh(y), sinh(y)), simultaneously.      */
    tmpl_LDouble_SinCos(z.dat[0], &sin_x, &cos_x);
    tmpl_LDouble_SinhCosh(z.dat[1], &sinh_y, &cosh_y);

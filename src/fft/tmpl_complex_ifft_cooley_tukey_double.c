                               ...
                               size_t N)
{
    ...
    const double one_by_N = 1.0 / (double)N;
    const double factor = 2.0 * one_by_N;
    ...
    for (k = 0; k<N; ++k)
        twiddles[k] = tmpl_CDouble_ExpiPi((double)k * factor);
    ...
    for (k=0; k<N; ++k)
        out[k] = tmpl_CDouble_Multiply_Real(one_by_N, out[k]);
}

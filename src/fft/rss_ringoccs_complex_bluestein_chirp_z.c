                                   ...
                                   unsigned long N, tmpl_Bool inverse)
{
    ...
    tmpl_ComplexDouble *fft_x_in;
    tmpl_ComplexDouble *fft_rcpr_chirp;
    ...
    for (n=0; n<chirp_size; ++n)
    {
        ...
        rcpr_chirp[n] = tmpl_CDouble_Reciprocal(chirp[n]);
    }
    ...
    tmpl_CDouble_FFT_Cooley_Tukey(x_in, fft_x_in, N_pow_2, tmpl_False);
    ...
    tmpl_CDouble_FFT_Cooley_Tukey(rcpr_chirp, fft_rcpr_chirp,
                                  N_pow_2, tmpl_False);
    ...
    for (n=0; n<N_pow_2; ++n)
        x_in[n] = tmpl_CDouble_Multiply(fft_x_in[n], fft_rcpr_chirp[n]);

    free(fft_x_in);
    tmpl_CDouble_FFT_Cooley_Tukey(x_in, fft_x_in, N_pow_2, tmpl_True);

    for(n=0; n<N; ++n)
    {
        m = n+N-1;
        out[n] = tmpl_CDouble_Multiply(fft_x_in[m], chirp[m]);
    }

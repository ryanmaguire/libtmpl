 ...
 ******************************************************************************
 *                           tmpl_chebyshev_t_double                          *
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Computes the evaluation of the Chebyshev T polynomials (first kind).  *
 ******************************************************************************
 *                             DEFINED FUNCTIONS                              *
 ******************************************************************************
 *  Function Name:                                                            *
 ...
 *  Purpose:                                                                  *
 *      Computes T_n(x) for a fixed x and for n = 0, 1, ..., length - 1.      *
 *  Arguments:                                                                *
 *      evals (double * const):                                               *
 *          The array of evaluations. evals[n] stores the value T_n(x).       *
 *      x (double):                                                           *
 *          A real number. Typically this is between -1 and 1.                *
 *      length (size_t):                                                      *
 *          The number of elements in the array. The highest Chebyshev        *
 *          polynomial that is computed is T_{length - 1}(x).                 *
 *  Output:                                                                   *
 *      None (void).                                                          *
 *  Called Functions:                                                         *
 *      None.                                                                 *
 ...
 *      In general, the error for T_n(x) is on the order of:                  *
 *                                                                            *
...
/*  Computes the Chebyshev polynomials T_n(x) for n = 0, 1, ..., length - 1.  */
void tmpl_Double_Chebyshev_T(double * const evals, double x, size_t length)
{
    ...
    /*  evals has at least one element allocated to it. T_0(x) = 1. Use this. */
    evals[0] = 1.0;

    /*  If length is not greater than two, we can skip the recursion          *
     *  formula. In this case, we are just computing T_0(x). Return.          */
    if (length == one)
        return;
...
/*  End of tmpl_Double_Chebyshev_T.                                           */

...
#ifndef TMPL_NORMALIZED_FRESNEL_SIN_AUXILIARY_FLOAT_H
#define TMPL_NORMALIZED_FRESNEL_SIN_AUXILIARY_FLOAT_H
...
TMPL_CONST_FUNC
TMPL_STATIC_INLINE
float tmpl_Float_Normalized_Fresnel_Sin_Auxiliary(const float x)
TMPL_UNSEQUENCED
{
    ...
    tmpl_Float_SinCosPi(TMPL_CAST(arg, float), &sin_x, &cos_x);

    /*  With the auxiliary functions computed, we can compute C(x).           */
    return 0.5F - (f * cos_x + g * sin_x);
}
/*  End of tmpl_Float_Normalized_Fresnel_Sin_Auxiliary.                       */

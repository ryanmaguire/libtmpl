...
#include "auxiliary/tmpl_normalized_fresnel_cos_auxiliary_ldouble.h"

/*  Computes C(x) using auxiliary functions "f" and "g" for 2 <= x < 4.       */
#include "auxiliary/tmpl_normalized_fresnel_cos_auxiliary_small_ldouble.h"

/*  Computes C(x) using an asymptotic expansion for x > 2^17.                 */
#include "auxiliary/tmpl_normalized_fresnel_cos_asymptotic_ldouble.h"

/*  Computes C(x) using an asymptotic expansion for very large x.             */
#include "auxiliary/tmpl_normalized_fresnel_cos_asymptotic_large_ldouble.h"

/*  Computes C(x) using a Maclaurin series for |x| < 1 / 4.                   */
#include "auxiliary/tmpl_normalized_fresnel_cos_maclaurin_ldouble.h"

/*  Computes C(x) using a Pade approximant for |x| < 1.                       */
#include "auxiliary/tmpl_normalized_fresnel_cos_pade_ldouble.h"

/*  Computes C(x) using a lookup table of Remez coefficients for 1 <= x < 2.  */
#include "auxiliary/tmpl_normalized_fresnel_cos_remez_ldouble.h"
...
long double tmpl_LDouble_Normalized_Fresnel_Cos(long double x)
{
    ...
    if (TMPL_LDOUBLE_EXPO_BITS(w) < TMPL_LDOUBLE_ASYMPTOTIC_EXPO)
    {
        ...
        if (TMPL_LDOUBLE_EXPO_BITS(w) == TMPL_LDOUBLE_UBIAS)
            out = tmpl_LDouble_Normalized_Fresnel_Cos_Remez(w);
    ...
    /*  For very large inputs use a single term of the asymptotic series.     */
        ...
        out = tmpl_LDouble_Normalized_Fresnel_Cos_Asymptotic(w.r);
    else if (TMPL_LDOUBLE_EXPO_BITS(w) < TMPL_LDOUBLE_LIMIT_EXPO)
        out = tmpl_LDouble_Normalized_Fresnel_Cos_Asymptotic_Large(w.r);

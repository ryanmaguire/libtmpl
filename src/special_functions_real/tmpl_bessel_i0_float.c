 ...
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Code for computing the Bessel I0 function at single precision.        *
 ******************************************************************************
 *                             DEFINED FUNCTIONS                              *
 ******************************************************************************
 *  Function Name:                                                            *
 ...
 *  Called Functions:                                                         *
 *      tmpl_Float_Bessel_I0_Maclaurin (tmpl_special_functions_real.h):       *
 *          Computes I0(x) using a Maclaurin series for small inputs.         *
 *      tmpl_Float_Bessel_I0_Asymptotic (tmpl_special_functions_real.h):      *
 *          Computes I0(x) using an asymptotic expansion for large inputs.    *
 *      tmpl_Float_Bessel_I0_Chebyshev (tmpl_special_functions_real.h):       *
 ...
 *          8 <= x < 32:  Chebyshev expansion                                 *
 *          64 < x < max: Asymptotic expansion                                *
 *          max <= x:     Return infinity                                     *
 *      Where max is the largest value for float such that exp(x) will not    *
 *      overflow. For 32-bit float this is roughly 88.                        *
 ******************************************************************************
 *                                DEPENDENCIES                                *
 ******************************************************************************
 *  1.) tmpl_math.h:                                                          *
...
/*  Computes the Modified Bessel function I0 at single precision.             */
float tmpl_Float_Bessel_I0(float x)
{
    ...
    if (w.bits.expo < TMPL_FLOAT_UBIAS + 3U)
        return tmpl_Float_Bessel_I0_Maclaurin(w.r);

    /*  For intermediate values use a Chebyshev expansion.                    */
    else if (w.bits.expo < TMPL_FLOAT_UBIAS + 6U)
        return tmpl_Float_Bessel_I0_Chebyshev(w.r);

    /*  For larger values use the asymptotic expansion.                       */
    else if (w.r < tmpl_Max_Float_Base_E)
        return tmpl_Float_Bessel_I0_Asymptotic(w.r);
...
float tmpl_Float_Bessel_I0(float x)
{
    ...
    /*  For small arguments, use a Taylor series to approximate I0(x).        */
    if (abs_x < 8.0F)
        return tmpl_Float_Bessel_I0_Maclaurin(abs_x);

    /*  For intermediate values use a Chebyshev expansion.                    */
    else if (abs_x < 32.0F)
        return tmpl_Float_Bessel_I0_Chebyshev(abs_x);

    /*  For larger values, use the asymptotic expansion.                      */
    else if (abs_x < tmpl_Max_Float_Base_E)
        return tmpl_Float_Bessel_I0_Asymptotic(abs_x);

 ...
 *  Arguments:                                                                *
 *      x (double):                                                           *
 *          A real number, the argument for S(x).                             *
 *  Output:                                                                   *
 *      S_x (double):                                                         *
 *          The normalized Fresnel sine function S(x).                        *
 *  Called Functions:                                                         *
...
TMPL_UNSEQUENCED
{
    ...
    if (w.bits.expo < TMPL_DOUBLE_UBIAS)
    {
        ...
        return tmpl_Double_Normalized_Fresnel_Sin_Pade(x);
    }
    ...
    if (w.bits.expo < TMPL_DOUBLE_UBIAS + 0x11U)
    {
        ...
        if (w.bits.expo == TMPL_DOUBLE_UBIAS)
            out = tmpl_Double_Normalized_Fresnel_Sin_Remez(w);
         ...
         *  auxiliary functions. This gives us a bit of a speed boost.        */
            ...
            out = tmpl_Double_Normalized_Fresnel_Sin_Auxiliary_Small(w.r);
...
double tmpl_Double_Normalized_Fresnel_Sin(double x)
{
    ...
    if (abs_x < 131072.0)
    {
         ...
         *  coefficients for Remez polynomials spaced 1/32 apart.             */
        if (abs_x < 2.0)
            out = tmpl_Double_Normalized_Fresnel_Sin_Remez(abs_x);

        /*  For 2 <= |x| < 4, less care is needed to accurately use the       *
         *  auxiliary functions. This gives us a bit of a speed boost.        */
        else if (abs_x < 4.0)
            out = tmpl_Double_Normalized_Fresnel_Sin_Auxiliary_Small(abs_x);
     ...
     *  are exact.                                                            */
        ...
        out = tmpl_Double_Normalized_Fresnel_Sin_Asymptotic_Large(abs_x);

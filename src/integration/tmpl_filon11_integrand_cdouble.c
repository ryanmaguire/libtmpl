 ...
 ******************************************************************************
 *                       tmpl_filon11_integrand_cdouble                       *
 ******************************************************************************
 *  Purpose:                                                                  *
                               ...
                               const double width)
{
    ...
    tmpl_ComplexDouble z_left, z_right, factor;
    ...
    const tmpl_ComplexDouble exp_left = tmpl_CDouble_Expi(g_left);
    const tmpl_ComplexDouble exp_right = tmpl_CDouble_Expi(g_right);
    ...
    const double g_diff = g_right - g_left;
    ...
    else
    {
        ...
        const double rcpr_slope = width / g_diff;

        /*  Next, compute (f(R) - f(L)) / (g(R) - g(L)).                      */
        factor = tmpl_CDouble_Subtract(right, left);
        tmpl_CDouble_MultiplyBy_Real(&factor, 1.0 / g_diff);

        /*  Next, compute exp(i g(L)) (i f(L) - a / c).                       */
        z_left = tmpl_CDouble_Multiply_Imag(1.0, left);
        tmpl_CDouble_SubtractFrom(&z_left, &factor);
        tmpl_CDouble_MultiplyBy(&z_left, &exp_left);

        /*  Now compute exp(i g(R)) (i f(R) - a / c).                         */
        z_right = tmpl_CDouble_Multiply_Imag(1.0, right);
        tmpl_CDouble_SubtractFrom(&z_right, &factor);
        tmpl_CDouble_MultiplyBy(&z_right, &exp_right);
        ...
        integrand = tmpl_CDouble_Subtract(z_left, z_right);
...
/*  End of tmpl_CDouble_Filon11_Integrand.                                    */

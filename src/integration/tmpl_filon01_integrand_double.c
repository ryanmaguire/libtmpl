                              ...
                              const double width)
{
    ...
    const tmpl_ComplexDouble left_exp = tmpl_CDouble_Expi(g_left);
    const tmpl_ComplexDouble right_exp = tmpl_CDouble_Expi(g_right);
    ...
    if (tmpl_Double_Abs(g_diff) < threshold)
    {
        ...
        integrand = tmpl_CDouble_Midpoint(left_exp, right_exp);
    ...
    else
    {
        ...
        integrand = tmpl_CDouble_Subtract(right_exp, left_exp);
        ...
        tmpl_CDouble_MultiplyBy_Imag(&integrand, -rcpr_slope);
    }

    /*  We treat f(t) as constant across the window. Scale the integral by    *
     *  this factor to complete the calculation.                              */
    tmpl_CDouble_MultiplyBy_Real(&integrand, value);
    return integrand;
}
/*  End of tmpl_Double_Filon01_Integrand.                                     */

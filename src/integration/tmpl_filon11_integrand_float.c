                             ...
                             const float width)
{
    ...
    tmpl_ComplexFloat z_left, z_right;
    ...
    if (tmpl_Float_Abs(g_diff) < threshold)
    {
        ...
        z_left = tmpl_CFloat_Multiply_Real(left, exp_left);
        z_right = tmpl_CFloat_Multiply_Real(right, exp_right);
    ...
    else
    {
        ...
        const float factor = (right - left) / g_diff;

        /*  Next, compute exp(i g(L)) (i f(L) - a / c).                       */
        z_left = tmpl_CFloat_Rect(-factor, left);
        tmpl_CFloat_MultiplyBy(&z_left, &exp_left);

        /*  Now compute exp(i g(R)) (i f(R) - a / c).                         */
        z_right = tmpl_CFloat_Rect(-factor, right);
...
/*  End of tmpl_Float_Filon11_Integrand.                                      */

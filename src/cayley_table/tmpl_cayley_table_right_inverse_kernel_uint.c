 ...
 ******************************************************************************
 *                tmpl_cayley_table_right_inverse_kernel_uint                 *
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Solves the equation element * x = 0 from a given Cayley table.        *
 ******************************************************************************
 *                             DEFINED FUNCTIONS                              *
 ******************************************************************************
 *  Function Name:                                                            *
 ...
 *  Purpose:                                                                  *
 *      Given a Cayley table, attempts to solve element * x = 0.              *
 *  Arguments:                                                                *
 *      table (const tmpl_UIntCayleyTable * const):                           *
 *          The Cayley table representing the algebraic structure.            *
 *      element (const unsigned int):                                         *
 *          The element we are trying to find a right-inverse of.             *
 *  Output:                                                                   *
 *      x (unsigned int):                                                     *
 *          A solution to element * x = 0 (if it exists).                     *
 *  Called Functions:                                                         *
 *      None.                                                                 *
 *  Method:                                                                   *
 *      Loop through all possible values "x" and check for element * x = 0.   *
...
/*  Searches for a right-inverse to a given element in a Cayley table.        */
unsigned int
tmpl_UIntCayleyTable_Right_Inverse_Kernel(
...
)
{
     ...
     *  search for solutions to element * x = 0.                              */
    size_t x;

    /*  Loop through the element row, searching for an x with element * x = 0.*/
    for (x = 0; x < table->size; ++x)
    {
        /*  Solving for element * x = 0 means we are looking for an index x   *
         *  with table[x, element] = 0. Check each element in the row.        */
        if (TMPL_CAYLEY_TABLE_ENTRY(table, element_ind, x) == 0)
            return TMPL_CAST(x, unsigned int);
    }

    /*  No solution was found, meaning element * x = 0 cannot be solved.      *
     ...
     *  have a right inverse in a general Cayley table.                       */
    return invalid;
}
/*  End of tmpl_UIntCayleyTable_Right_Inverse_Kernel.                         */

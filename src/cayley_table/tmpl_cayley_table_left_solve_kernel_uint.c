 ...
 ******************************************************************************
 *                  tmpl_cayley_table_left_solve_kernel_uint                  *
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Solves the equation x * a = b from a given Cayley table.              *
 ******************************************************************************
 *                             DEFINED FUNCTIONS                              *
 ******************************************************************************
 *  Function Name:                                                            *
 ...
 *      None.                                                                 *
 *  Method:                                                                   *
 *      Loop through all possible values "x" and check for x * a = b.         *
...
)
{
     ...
     *  search for solutions to x * a = b.                                    */
    size_t x;

    /*  Loop through the "a" row, searching for an "x" with x * a = b.        */
    for (x = 0; x < table->size; ++x)
    {
        /*  Solving for x * a = b means we are looking for an index x with    *
         *  table[x, a] = b. Check each element in the row.                   */
        if (TMPL_CAYLEY_TABLE_ENTRY(table, x, a_ind) == b_ind)
            return TMPL_CAST(x, unsigned int);
    }

    /*  No solution was found, meaning x * a = b cannot be solved. Return     *
     ...
     *  inverses exist. It is certainly possible for x * a = b to have no     *
     *  solution for a general Cayley table.                                  */
    return invalid;
}
/*  End of tmpl_UIntCayleyTable_Left_Solve_Kernel.                            */

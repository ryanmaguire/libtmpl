 ...
 ******************************************************************************
 *              tmpl_cayley_table_is_left_self_distributive_uint              *
 ******************************************************************************
 *  Purpose:                                                                  *
 ...
 *      tmpl_UIntCayleyTable_Is_Left_Self_Distributive_Kernel, which checks   *
 *      if x*(y*z) = (x*y)*(x*z) for all possible triples from the set.       *
 *  Notes:                                                                    *
...
)
{
     ...
     *  "empty operation" on the empty set. It is vacuously a left            *
     *  self-distributive table (there are no elements x, y, z in the empty   *
     *  set with x*(y*z) != (x*y)*(x*z), so the operation is indeed a left    *
    ...
    if (!table->data)
    {
         ...
         *  the empty set. The operation is left self-distributive.           */
        if (table->size == 0)
            return tmpl_True;

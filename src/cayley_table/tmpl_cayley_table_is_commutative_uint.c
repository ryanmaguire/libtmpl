 ...
 ******************************************************************************
 *                    tmpl_cayley_table_is_commutative_uint                   *
 ******************************************************************************
 *  Purpose:                                                                  *
 ...
 *              Check if x * y = y * x for all pairs (x, y) from the set.     *
 *  Method:                                                                   *
 ...
 *      tmpl_UIntCayleyTable_Is_Commutative_Kernel, which checks if           *
 *      x * y = y * x for all possible pairs from the set.                    *
 *  Notes:                                                                    *
...
tmpl_UIntCayleyTable_Is_Commutative(const tmpl_UIntCayleyTable * const table)
{
     ...
     *  "empty operation" on the empty set. It is vacuously a commutative     *
     *  table (there are no elements x, y in the empty set with x*y != y*x).  *
     *                                                                        *
    ...
    if (!table->data)
    {
         ...
         *  working with the empty set. The operation is hence commutative.   */
        if (table->size == 0)
            return tmpl_True;
    ...
    /*  Perform the main check, looping through pairs (x, y) and checking     *
     *  for x * y != y * x.                                                   */
    return tmpl_UIntCayleyTable_Is_Commutative_Kernel(table);
}
/*  End of tmpl_UIntCayleyTable_Is_Commutative.                               */

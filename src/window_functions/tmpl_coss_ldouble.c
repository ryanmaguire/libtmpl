...
#include "auxiliary/tmpl_coss_remez_ldouble.h"

/*  Rational Remez approximation for the squared cosine window.               */
#include "auxiliary/tmpl_coss_rat_remez_ldouble.h"

/*  Tail-end expansion using a shifted rational Remez approximation.          */
#include "auxiliary/tmpl_coss_tail_end_ldouble.h"
...
/*  Long double precision squared cosine window.                              */
long double tmpl_LDouble_Coss(const long double x, const long double width)
{
    ...
    if (TMPL_LDOUBLE_EXPO_BITS(w) < TMPL_LDOUBLE_UBIAS - 0x02U)
    {
        ...
        if (TMPL_LDOUBLE_EXPO_BITS(w) < TMPL_LDOUBLE_UBIAS - 0x05U)
        {
            ...
            return tmpl_LDouble_Coss_Remez(w.r);
        }

        /*  For 1 / 32 <= |x| < 1 / 4, use a rational Remez approximation.    */
        return tmpl_LDouble_Coss_Rat_Remez(w.r);
    }
    ...
    if (TMPL_LDOUBLE_EXPO_BITS(w) < TMPL_LDOUBLE_UBIAS - 0x01U)
    {
         ...
         *  terms of x - 1 / 2.                                               */
        return tmpl_LDouble_Coss_Tail_End(w.r);
    }
...
long double tmpl_LDouble_Coss(const long double x, const long double width)
{
    ...
    if (abs_arg < 0.25L)
    {
        ...
        if (abs_arg < 0.03125L)
        {
             ...
             *  O(x^2), the value of the window is 1 to extended precision.   */
            if (abs_arg < 5.82076609134674072265625E-11L)
                return 1.0L;

            /*  Otherwise, use a Remez polynomial with a few terms.           */
            return tmpl_LDouble_Coss_Remez(abs_arg);
        }

        /*  For 1 / 32 <= |x| < 1 / 4, use a rational Remez approximation.    */
        return tmpl_LDouble_Coss_Rat_Remez(abs_arg);
    }
     ...
     *  endpoints.                                                            */
    if (abs_arg < 0.5L)
        return tmpl_LDouble_Coss_Tail_End(abs_arg);

 ...
 *  Arguments:                                                                *
 *      f (const tmpl_AffineFloat * const):                                   *
 *          An affine transformation, f(x) = a * x + b.                       *
 *      g (const tmpl_AffineFloat * const):                                   *
...
TMPL_UNSEQUENCED
{
    ...
    const float intercept = f->dat[1] + g->dat[1];

    /*  Given the slope and y-intercept, we can construct the transformation  *
     *  using slope-intercept form.                                           */
    return tmpl_AffFloat_Slope_Intercept(slope, intercept);
}
/*  End of tmpl_AffFloat_Add.                                                 */

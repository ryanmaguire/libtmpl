...
TMPL_UNSEQUENCED
{
    ...
    const long double intercept = f->dat[1] * g->dat[0] + g->dat[1];
    const long double slope = f->dat[0] * g->dat[0];

    /*  The transformation can be constructed from slope-intercept form.      */
    return tmpl_AffLDouble_Slope_Intercept(slope, intercept);
}
/*  End of tmpl_AffLDouble_Compose.                                           */

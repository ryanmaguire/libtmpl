...
#define TMPL_SQRT_3_BY_2 (+1.224744871391589049098642037352945695983E+00)
...
tmpl_3DDouble_Orthogonal(const tmpl_ThreeVectorDouble * const P)
{
    ...
    const double a = P->dat[1] + P->dat[2];
    const double b = P->dat[2] - P->dat[0];
    const double c = -P->dat[0] - P->dat[1];
    const double norm = tmpl_Double_Hypot3(a, b, c);
    ...
    else
    {
        ...
        const double factor = tmpl_3DDouble_L2_Norm(P) / norm;
        out.dat[0] = factor * a;
        out.dat[1] = factor * b;
        out.dat[2] = factor * c;
    }
...
/*  End of tmpl_3DDouble_Orthogonal.                                          */

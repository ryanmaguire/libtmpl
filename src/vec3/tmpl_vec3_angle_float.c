 ...
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Contains code for computing angles at single precision.               *
 ******************************************************************************
 *                             DEFINED FUNCTIONS                              *
 ******************************************************************************
 *  Function Name:                                                            *
 ...
 *          tmpl_3DFloat_Quick_Angle. That function does not safely handle    *
 ...
 *          Location of the TMPL_HAS_IEEE754_FLOAT macro.                     *
 ******************************************************************************
 *  Author:     Ryan Maguire                                                  *
 *  Date:       June 10, 2024                                                 *
 ******************************************************************************/
...
/*  The TMPL_HAS_IEEE754_FLOAT macro is found here.                           */
...
#define TMPL_2_POW_512 (+1.8446744073709551616E+19F)
                   ...
                   const tmpl_ThreeVectorFloat * const q)
{
    ...
    float dot, norm_cross;

    /*  To avoid overflows and undeflows we scale the inputs. Make a copy.    */
    tmpl_ThreeVectorFloat pn = *p;
    tmpl_ThreeVectorFloat qn = *q;
    ...
    const unsigned int p_max_exp = TMPL_VEC3_FLOAT_MAX_EXP(pn);
    const unsigned int q_max_exp = TMPL_VEC3_FLOAT_MAX_EXP(qn);

    /*  If || p || > 2^61, scale the components by 2^-64. This guarantees     *
     *  that the scaled result will not produce overflows in the intermediate *
     *  calculations (cross product, dot product). We choose 2^61 as the      *
     *  threshold since 61 = 64 - 3. If each of the components is roughly     *
     *  2 * 2^61, then the sum of the squares is 3 * 4 * 2^122 ~= 2^125,      *
     *  which safely fits within the range of float. Larger values may        *
     *  overflow, so we scale those ones down.                                */
    if (p_max_exp > TMPL_FLOAT_UBIAS + 0x3DU)
        tmpl_3DFloat_ScaleBy(&pn, 1.0F / TMPL_2_POW_512);

    /*  Similarly, if || p || < 2^-61, then scale by 2^64. This prevents      *
     *  underflow from occurring in the intermediate steps.                   */
    else if (p_max_exp < TMPL_FLOAT_UBIAS - 0x3DU)
        tmpl_3DFloat_ScaleBy(&pn, TMPL_2_POW_512);

    /*  Same check for || q ||.                                               */
    if (q_max_exp > TMPL_FLOAT_UBIAS + 0x3DU)
        tmpl_3DFloat_ScaleBy(&qn, 1.0F / TMPL_2_POW_512);
    else if (q_max_exp < TMPL_FLOAT_UBIAS - 0x3DU)
        tmpl_3DFloat_ScaleBy(&qn, TMPL_2_POW_512);

    /*  From vector geometry, || p x q || = || p || || q || sin(theta).       *
     *  Computing the norm of || p x q || gives us a formula for the angle.   */
    cross = tmpl_3DFloat_Cross_Product(&pn, &qn);

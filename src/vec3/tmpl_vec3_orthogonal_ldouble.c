...
#define TMPL_SQRT_3_BY_2 (+1.224744871391589049098642037352945695983E+00L)
...
tmpl_3DLDouble_Orthogonal(const tmpl_ThreeVectorLongDouble * const P)
{
    ...
    tmpl_ThreeVectorLongDouble out;

    /*  (y + z, z - x, -x - y) is always orthogonal to (x, y, z), but it may  *
     *  be the zero vector. We'll check for this.                             */
    const long double a = P->dat[1] + P->dat[2];
    const long double b = P->dat[2] - P->dat[0];
    const long double c = -P->dat[0] - P->dat[1];
    const long double norm = tmpl_LDouble_Hypot3(a, b, c);
    ...
    else
    {
        ...
        const long double factor = tmpl_3DLDouble_L2_Norm(P) / norm;
        out.dat[0] = factor * a;
        out.dat[1] = factor * b;
        out.dat[2] = factor * c;
    }
...
/*  End of tmpl_3DLDouble_Orthogonal.                                         */

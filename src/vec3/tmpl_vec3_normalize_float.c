...
tmpl_3DFloat_Normalize(const tmpl_ThreeVectorFloat * const P)
{
    ...
    if (max_expo > TMPL_FLOAT_UBIAS + 0x40U)
    {
    ...
    else if (max_expo < TMPL_FLOAT_UBIAS - 0x34U)
    {
        /*  If the components are denormal, normalize them.                   */
        if (max_expo == 0x00U)
        {
            wx.r *= TMPL_FLOAT_NORMALIZE;
            wy.r *= TMPL_FLOAT_NORMALIZE;
            wz.r *= TMPL_FLOAT_NORMALIZE;
        }
    ...
    norm = tmpl_Float_Sqrt(wx.r*wx.r + wy.r*wy.r + wz.r*wz.r);
 ...
 *  not catch certain overflows. For example, is x = y = z = FLT_MAX, this    *
 *  should normalize to (1/sqrt(3)) * (1, 1, 1). But the norm of (x, y, z) is *
 *  sqrt(3) * FLT_MAX, which will overflow. Because of this the output will   *
...
tmpl_3DFloat_Normalize(const tmpl_ThreeVectorFloat * const P)
{
    ...
    const float rcpr_norm = 1.0F / norm;

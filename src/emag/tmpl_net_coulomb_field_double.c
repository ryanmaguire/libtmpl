...
tmpl_3DDouble_Net_Coulomb_Field(const tmpl_ThreeVectorDouble * const observer,
...
TMPL_UNSEQUENCED
{
    ...
    tmpl_ThreeVectorDouble field = tmpl_3DDouble_Zero;
    tmpl_ThreeVectorDouble error = tmpl_3DDouble_Zero;
    ...
    for (n = 0; n < number_of_charges; ++n)
    {
         ...
         *  point charge.                                                     */
        const tmpl_ThreeVectorDouble point_charge_e_field =
            tmpl_3DDouble_Coulomb_Field(observer, &charges[n]);

        /*  Add this contribution to the running sum.                         */
        tmpl_3DDouble_Neumaier_Two_Sum(&point_charge_e_field, &field, &error);
    }

    /*  With Neumaier 2Sum it is possible for || error || > || field ||. The  *
     *  correct summation is given by summing together the running sum with   *
     *  the error term.                                                       */
    return tmpl_3DDouble_Add(&field, &error);
}
/*  End of tmpl_3DDouble_Net_Coulomb_Field.                                   */

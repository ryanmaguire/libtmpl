...
TMPL_UNSEQUENCED
{
    ...
    tmpl_ThreeVectorLongDouble field = tmpl_3DLDouble_Zero;
    tmpl_ThreeVectorLongDouble error = tmpl_3DLDouble_Zero;
    ...
    for (n = 0; n < number_of_charges; ++n)
    {
         ...
         *  point charge.                                                     */
        const tmpl_ThreeVectorLongDouble point_charge_e_field =
            tmpl_3DLDouble_Coulomb_Field(observer, &charges[n]);

        /*  Add this contribution to the running sum.                         */
        tmpl_3DLDouble_Neumaier_Two_Sum(&point_charge_e_field, &field, &error);
    }

    /*  With Neumaier 2Sum it is possible for || error || > || field ||. The  *
     *  correct summation is given by summing together the running sum with   *
     *  the error term.                                                       */
    return tmpl_3DLDouble_Add(&field, &error);
}
/*  End of tmpl_3DLDouble_Net_Coulomb_Field.                                  */

...
TMPL_UNSEQUENCED
{
    ...
    long double dist, dist_cubed, scale;
    ...
    diff = tmpl_3DLDouble_Subtract(observer, &charge->pos);
    ...
    dist = tmpl_3DLDouble_L2_Norm(&diff);
    dist_cubed = dist * dist * dist;

    /*  From Coulomb's law the scale is k * q / r^3.                          */
    scale = tmpl_ldouble_coulomb_constant * charge->charge / dist_cubed;
    ...
    return tmpl_3DLDouble_Scale(scale, &diff);
}
/*  End of tmpl_3DLDouble_Coulomb_Field.                                      */

...
tmpl_Float_Cyl_Fresnel_Scale(float lambda, float D, float phi, float B)
{
    ...
    const float sin_phi = tmpl_Float_Sin(phi);

    /*  Compute cos(B) and sin(B) simultaneously with SinCos.                 */
    tmpl_Float_SinCos(B, &sin_B, &cos_B);
...
/*  End of tmpl_Float_Cyl_Fresnel_Scale.                                      */

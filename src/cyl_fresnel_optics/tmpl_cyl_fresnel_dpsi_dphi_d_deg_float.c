 ...
 ******************************************************************************
 *                   tmpl_cyl_fresnel_dpsi_dphi_d_deg_float                   *
 ******************************************************************************
 *  Purpose:                                                                  *
 *      Computes the first partial derivative of the Cylindrical Fresnel      *
 *      kernel with respect to phi at single precision. The distance D from   *
 ...
 ******************************************************************************
 *  Function Name:                                                            *
 *      tmpl_Float_Cyl_Fresnel_dPsi_dPhi_D_Deg                                *
 *  Purpose:                                                                  *
 *      Computes the first partial derivative of the cylindrical Fresnel      *
                                       ...
                                       float rx, float ry, float rz)
{
    ...
    float xi, eta, psi0, dxi, deta, rcpr_D, rcpr_D_squared;
    float cos_phi, sin_phi, cos_phi0, sin_phi0, cos_phi_phi0, sin_phi_phi0;
    float dpsi, dD, D, dx, dy, xi_factor, eta_factor;
    ...
    dx = rx - r*cos_phi;
    dy = ry - r*sin_phi;
    D = tmpl_Float_Sqrt(dx*dx + dy*dy + rz*rz);
    ...
    deta = eta_factor * sin_phi_phi0 - 2.0F * eta * rcpr_D * dD;
    dpsi = k * D * ((0.5F/psi0) * (deta - 2.0F*dxi) + dxi);
    dpsi += k * dD * (psi0 + xi - 1.0F);

    return dpsi;
}
/*  End of tmpl_Float_Cyl_Fresnel_dPsi_dPhi_D_Deg.                            */

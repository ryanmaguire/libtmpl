 ...
 ******************************************************************************
 *           tmpl_ideal_stationary_cyl_fresnel_phi_newton_deg_float           *
 ******************************************************************************
 *  Purpose:                                                                  *
                                                       ...
                                                       unsigned int max_iters)
{
    ...
    float xi, eta, psi0, dxi, deta, deta2, dxi2, rcpr_psi0, rcpr_psi0_cubed;
    float cos_phi, sin_phi, cos_phi0, sin_phi0, cos_phi_phi0, sin_phi_phi0;
    float dpsi, d2psi, err, num_factor;

    /*  Index for keeping track of the number of iterations.                  */
    unsigned int n = 0U;

    /*  The functions tmpl_Float_Ideal_Cyl_Fresnel_d2Psi_dPhi2 and            *
     *  tmpl_Float_Ideal_Cyl_Fresnel_dPsi_dPhi do a lot of the same           *
    ...
    const float eta_factor = 2.0F * rho * rho0 * rcpr_D_squared;

    /*  Compute sine and cosine simultaneously using SinCosd.                 */
    tmpl_Float_SinCosd(phi0, &sin_phi0, &cos_phi0);
    ...
    do {
        ...
        dpsi = 0.5F * rcpr_psi0 * num_factor + dxi;
        ...
        d2psi = -0.25F * rcpr_psi0_cubed * num_factor * num_factor;
        d2psi += 0.5F * rcpr_psi0 * (deta2 - 2.0F * dxi2) + dxi2;
        ...
        phi = phi - tmpl_float_rad_to_deg * dpsi / d2psi;
        ++n;

        /*  Update the error. We want the partial derivative to be small.     */
        err = tmpl_Float_Abs(dpsi);

    } while (err > eps);

    return phi;
}
/*  End of tmpl_Float_Ideal_Stationary_Cyl_Fresnel_Phi_Newton_Deg.            */

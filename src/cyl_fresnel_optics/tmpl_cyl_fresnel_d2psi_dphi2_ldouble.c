 ...
 ******************************************************************************
 *                    tmpl_cyl_fresnel_d2psi_dphi2_ldouble                    *
 ******************************************************************************
 *  Purpose:                                                                  *
...
)
{
    ...
    const long double rcpr_rho_dist = 1.0L / rho_dist;
    const long double rcpr_rho_dist_sq = rcpr_rho_dist * rcpr_rho_dist;
    ...
    const long double diff = rho->dat[0] * R->dat[1] - rho->dat[1] * R->dat[0];
    const long double sum = R->dat[0] * rho->dat[0] + R->dat[1] * rho->dat[1];

    /*  Call the combination of the left and center parts "left". Compute.    */
    const long double left = (sum - diff*diff*rcpr_rho_dist_sq) * rcpr_rho_dist;

    /*  The expression u . rho'' is -(ux * rho_x + uy * rho_y). The right     *
     *  expression is this factor divided by the distance from R to rho0.     */
    const long double right = -(ux*rho->dat[0] + uy*rho->dat[1]) / rho0_dist;
...
/*  End of tmpl_LDouble_Cyl_Fresnel_d2Psi_dPhi2.                              */

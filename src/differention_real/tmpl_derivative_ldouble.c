...
tmpl_LDouble_Derivative(long double (*f)(long double), long double x)
{
    ...
    const long double eps = TMPL_SQRT_LDBL_EPS;
    const long double h = (abs_x > 1.0L ? abs_x * eps : eps);

    /*  The bulk of the work is done by the five-point stencil function.      */
    return tmpl_LDouble_Five_Point_Derivative(f, x, h);
}
/*  End of tmpl_LDouble_Derivative.                                           */

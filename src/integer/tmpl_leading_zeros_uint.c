...
int tmpl_UInt_Leading_Zeros(unsigned int n)
{
    /*  Variable for the number of leading zeros.                             */
    int bits = 0;
    const unsigned int zero = 0x00U;
    const unsigned int top16 = ~((~zero) >> 16U);
    const unsigned int top8 = ~((~zero) >> 8U);
    const unsigned int top4 = ~((~zero) >> 4U);
    const unsigned int top2 = ~((~zero) >> 2U);
    const unsigned int top1 = ~((~zero) >> 1U);

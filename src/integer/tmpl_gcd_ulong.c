...
/*  Function for computing the GCD of two unsigned long integers.             */
TMPL_CONST_FUNC
TMPL_ALWAYS_INLINE
unsigned long int
...
TMPL_UNSEQUENCED
{
    ...
    return tmpl_ULong_GCD_Mixed_Binary(m, n);
#elif TMPL_GCD_ALGORITHM == TMPL_GCD_ALGORITHM_BINARY
    return tmpl_ULong_GCD_Binary(m, n);
#elif TMPL_GCD_ALGORITHM == TMPL_GCD_ALGORITHM_EUCLIDEAN
    return tmpl_ULong_GCD_Euclidean(m, n);
#else
    return tmpl_ULong_GCD_Naive(m, n);
#endif
}

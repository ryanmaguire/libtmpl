...
/*  Routine for testing tmpl_CLDouble_Add_Real.                               */
int main(void)
{
    ...
    puts("Functions: tmpl_CLDouble_Add_Real vs complex addition");
    t1 = clock();
    for (x = 0U; x < N; ++x)
        for (y = 0U; y < N; ++y)
            z1[x][y] = tmpl_CLDouble_Add_Real(real, z0[x][y]);
    t2 = clock();

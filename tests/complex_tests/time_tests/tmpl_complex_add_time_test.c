...
/*  Routine for testing tmpl_CDouble_Add.                                     */
int main(void)
{
    ...
    const tmpl_ComplexDouble z = tmpl_CDouble_Rect(1.0, 2.0);
    const complex double w = 1.0 + (complex double)_Complex_I*2.0;
    ...
    puts("Functions: tmpl_CDouble_Add vs complex addition");
    t1 = clock();
    for (x = 0U; x < N; ++x)
        for (y = 0U; y < N; ++y)
            z1[x][y] = tmpl_CDouble_Add(z0[x][y], z);
    t2 = clock();

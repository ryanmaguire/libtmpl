...
int main(void)
{
    ...
    t1 = clock();
    for (n = 0U; n < N; ++n)
        tmpl_3DFloat_AddTo(&X[n], &Y[n]);
    t2 = clock();
    printf("libtmpl: %e\n", (double)(t2 - t1)/(double)CLOCKS_PER_SEC);

    t1 = clock();
    for (n = 0U; n < N; ++n)
        Vector3D_Add_flt32(&A[n], &B[n]);
    t2 = clock();
    printf("linasm:  %e\n", (double)(t2 - t1)/(double)CLOCKS_PER_SEC);

    for (n = 0U; n < N; ++n)
    {
        tmp = fabsf(X[n].dat[0] - A[n].x);
        x_rms += tmp*tmp;

        if (x_max < tmp)
            x_max = tmp;

        tmp = fabsf(X[n].dat[1] - A[n].y);
        y_rms += tmp*tmp;

        if (y_max < tmp)
            y_max = tmp;

        tmp = fabsf(X[n].dat[2] - A[n].z);
        z_rms += tmp*tmp;

...
/*  tmpl_Double_Is_Inf function found here.                                   */
...
int main(void)
{
    ...
    /*  And a variable for storing the output of tmpl_Double_Is_Inf.          */
    tmpl_Bool output;
    ...
    /*  The values that tmpl_Double_Is_Inf should return.                     */
    out[0] = tmpl_False;
    out[1] = tmpl_False;
    out[2] = tmpl_False;
    out[3] = tmpl_False;
    ...
    /*  Loop over the input array and test the tmpl_Double_Is_Inf function.   */
    for (n = 0U; n < sizeof(in)/sizeof(in[0]); ++n)
    {
        /*  Compute tmpl_Double_Is_Inf(in[n]) and compare with the out array. */
        output = tmpl_Double_Is_Inf(in[n]);
